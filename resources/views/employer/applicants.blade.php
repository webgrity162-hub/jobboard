...
@section('employer-content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">Applicants / View Applicants
                </p>
            ...
            <div class="border-b border-[#e8eef8] px-4 py-4 sm:px-6">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                    ...
                    <div class="relative min-w-[180px]">
                        <select
                            class="w-full appearance-none rounded-xl border border-[#d6deeb] bg-[#f9fbff] px-4 py-3 pr-10 text-sm text-slate-600 outline-none transition focus:border-linkedin-blue focus:ring-4 focus:ring-linkedin-blue/10"
                            name="job" id="job">
                            <option value="">Filter by Job: All</option>
                            @foreach ($jobs as $job)
                                <option value="{{ $job->slug }} " {{ request('job_slug') == $job->slug ? 'selected' : '' }}>{{ $job->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative min-w-[180px]">
                        <select
                            ...
                            <option value="" {{ request('status') == '' ? 'selected' : '' }}>Filter by Status: All
                            </option>
                    ...
                    <div class="relative min-w-[220px] flex-1 xl:min-w-[280px]">
                        ...
                        <input type="text" placeholder="Search candidate name..." id="search"
                            class="w-full rounded-xl border border-[#d6deeb] bg-[#f9fbff] py-3 pl-10 pr-4 text-sm text-slate-600 outline-none transition focus:border-linkedin-blue focus:ring-4 focus:ring-linkedin-blue/10">
                    </div>
    ...
    <script>
        function fetchApplication(page = 1) {
            ...
            fetch(`/employer/applicants?job_slug=${jobSlug}&status=${status}&search=${search}&page=${page}`, {
                ...
                .then(res => res.text())
                .then(html => {
                    document.getElementById('applicats-container').innerHTML = html
                });
